function Serversidetable_WorkHeight(Id) {
    let table = $(UI_Fields.MAINTABLE).DataTable({
        ...
        paging: true,
        ajax: {
            url: '/ControlOfWork/Get_All_Work_Height',
            type: 'POST',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (data == "Zone Supervisor Approval Pending") {
                        ...
                        html += '<a onclick="Fn_WAH_Report_Req(' + row.CSP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Confined Space Permit Report" class="text-info" data-bs-original-title="Work At Height Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    ...
                    if (data == "Zone Supervisor Rejected") {
                        html += '<a onclick="Fn_Edit_WAH(' + row.CSP_Id + ')" href="javascript:void(0);" title="Review" class="text-danger" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    ...
                    if (data == "Closed") {
                        ...
                        html += '<a onclick="Fn_WAH_Report(' + row.CSP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Confined Space Permit Report" class="text-info" data-bs-original-title="Work At Height Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    }
...
function Fn_View(val) {
    ...
    $(UI_Fields.WAH_REQUEST_VIEW).show(100);
    Hzurl = '/ControlOfWork/_ViewWorkHeight';
    Hzurl += '/?CSP_Id=' + val;
    $(UI_Fields.VIEW_WAH_LIST).load(Hzurl);
}
...
function Fn_Edit_WAH(val) {
    $(UI_Fields.LIST_VIEW).hide(100);
    $(UI_Fields.ADD_WAH_PERMIT).show(100);
    $(UI_Fields.GOOGLE_MAP_API).show(100);
    Load_GoogleMaps();
    $.post("/ControlOfWork/Edit_Work_Height", { ID: val }, function (data) {
        if (data != null) {
            $("#Work_Height_ID").val(data.CSP_Id);
            //$("#C_Date_Time_Obs").val(data.Date_Time_of_Observation);
            WAH_Get_Edit_Business_Unit("#Wah_Business_Unit_Id", data.Business_Unit_Id);
            WAH_Get_Edit_Zone_Unit("#Wah_Zone_Id", data.Zone_Id, data.Community_Id, data.Building_Id);
            WAH_Get_Edit_DMS_No_Edit("#Contractor_Id", data.DMS_No_Id);
            WAH_Get_Comp_Person_Edit("#C_Competent", data.Competent_Person, data.DMS_No_Id, "#Wah_Building_Id", data.Building_Id);        
...
function Fn_WAH_Report_Req(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Csp_ID = parseInt(id);
    $.post("/ControlofWorkReport/Work_Height_Request_Report", { CSP_Id: Csp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
...
function Fn_WAH_Report(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Csp_ID = parseInt(id);
    $.post("/ControlofWorkReport/Work_Height_Report", { CSP_Id: Csp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
