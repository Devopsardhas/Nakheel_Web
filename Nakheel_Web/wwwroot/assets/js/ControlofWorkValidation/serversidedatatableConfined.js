function Serversidetable_Confined(Id) {
    let table = $(UI_Fields.MAINTABLE).DataTable({
        ...
        paging: true,
        ajax: {
            url: '/ControlOfWork/GetAll_Confined_Permit_Work',
            type: 'POST',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (data == "Zone Supervisor Approval Pending") {
                        ...
                        html += '<a onclick="Fn_Csp_Report_Req(' + row.CSP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Confined Space Permit Report" class="text-info" data-bs-original-title="Confined Space Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';

                    }
                    if (data == "HSE Staff/HSE Team Approval Pending") {
                        html += '<a onclick="Fn_View_Confine_Hse(' + row.CSP_Id + ')"href="javascript:void(0);" title="Review" class="text-primary" data-bs-original-title="View" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';

                    }
                    if (data == "Zone Supervisor Rejected") {
                        html += '<a onclick="Fn_Edit_Confined(' + row.CSP_Id + ')" href="javascript:void(0);" title="Review" class="text-danger" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    if (data == "HSE Staff/HSE Team Approved") {
                        html += '<a onclick="Fn_View_Confine_Hse(' + row.CSP_Id + ')" href="javascript:void(0);" title="Review" class="text-success" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                        html += '<a onclick="Fn_Csp_Report_Req(' + row.CSP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Confined Space Permit Report" class="text-info" data-bs-original-title="Confined Space Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    }
                    ...
                    if (data == "Closed") {
                        ...
                        html += '<a onclick="Fn_Csp_Report(' + row.CSP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Confined Space Permit Report" class="text-info" data-bs-original-title="Confined Space Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    }
...
function Fn_View_Confine_Hse(val) {
...
function Fn_Edit_Confined(val) {
    debugger;
    $(UI_Fields.LIST_VIEW).hide(100);
    $(UI_Fields.ADD_CONFINED_SPACE_PERMIT).show(100);
    $(UI_Fields.GOOGLE_MAP_API).show(100);
    Load_GoogleMaps();
    $.post("/ControlOfWork/_EditConfinedSpace", { ID: val }, function (data) {
        debugger;
        if (data != null) {
            debugger;
            $("#Confined_ID").val(data.CSP_Id);
            $("#C_Date_Time_Obs").val(data.Date_Time_of_Observation);
            Get_Edit_Business_Unit("#Business_Unit_Id", data.Business_Unit_Id);
            Get_Edit_Zone_Unit("#Zone_Id", data.Zone_Id, data.Community_Id, data.Building_Id);
            Get_Edit_DMS_No_Edit("#Contractor_Id", data.DMS_No_Id);
            Get_Comp_Person_Edit("#C_Competent", data.Competent_Person, data.DMS_No_Id, "#Building_Id", data.Building_Id);        
            ...
            //Edit_Google_Map(data.Latitude, data.Longitude, data.Location_Address, data.Exact_Loc_Address)
            ...
            $("input[name=C_Night_Shedule][value=" + data.Night_Schedule + "]").attr('checked', 'checked');
...
function Fn_Csp_Report_Req(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Csp_ID = parseInt(id);
    $.post("/ControlofWorkReport/CSP_Requester_Details", { CSP_Id: Csp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
...
function Fn_Csp_Report(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Csp_ID = parseInt(id);
    $.post("/ControlofWorkReport/Confined_Space_Report", { CSP_Id: Csp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
