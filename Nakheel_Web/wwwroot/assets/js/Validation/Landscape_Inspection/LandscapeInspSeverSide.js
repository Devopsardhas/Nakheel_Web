function ServerSideTable_Landscape() {
    debugger
    let table = $(UI_Fields.MainTable).DataTable({
        ...
        ajax: {
            url: '/Inspection/Landscape_Inspection_GetAll',
            type: 'POST',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (data == "Corrective Action Pending" || data == "Review Pending") {
                        ...
                        html += '<a onclick="Fn_Insp_Land_Report(' + row.Insp_Request_Id + ',' + "'" + row.Unique_Id + "'" + ')" style="cursor: pointer;"><i style="color: red;font-size: 18px" class="mdi mdi-file-pdf"></i></a>';
                    }
...
function Fn_Insp_Land_Report(id, Unique_Id) {
    let NotiID = parseInt(id);
    $(".preloader").show();
    $("#List_View_Div").hide();
    $.post("/InspReport/Insp_Landscape_Report", { Insp_Request_Id: NotiID, Unique_Id: Unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
