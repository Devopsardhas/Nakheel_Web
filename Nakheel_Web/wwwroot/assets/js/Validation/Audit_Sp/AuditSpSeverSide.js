    ...
    let table = $(UI_Fields.MainTable).DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.columns = [
                    ...
                    { data: 'Audit_Sp_Date', search: { value: $('#MainTable_wrapper').find('thead input:eq(5)').val() } },
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    let html = "";
                    if (data == "Schedule Initiated" || data == "Audit Scheduled") {
                        if (row.Access_Schedule == "All") {
                            if (row.Role_Id == "9" || row.Role_Id == "10" || row.Role_Id == "11") {
                                html += '<a onclick="Edit(' + row.Audit_Sp_Sch_Id + ',' + "'" + 'Edit' + "'" + ',' + "'" + 'Access' + "'" + ')" href="javascript:void(0);" title="Edit" class="text-success" data-bs-original-title="Edit" aria-label="View" style="cursor: pointer;"> <i class="mdi mdi-pencil font-size-18"></i></a>';
                            }
                            else {
                                html += '<a onclick="Edit(' + row.Audit_Sp_Sch_Id + ',' + "'" + 'View' + "'" + ',' + "'" + 'NoAccess' + "'" + ')" href="javascript:void(0);" title="Edit" class="text-success" data-bs-original-title="Edit" aria-label="View" style="cursor: pointer;"> <i class="mdi mdi-eye font-size-18"></i></a>';
                            }
                        } else {
                        ...
                        html += '<a onclick="Fn_Audit_Sp_Report(' + row.Audit_Sp_Sch_Id + ',' + "'" + row.Unique_Id + "'" + ')" style="cursor: pointer;"><i style="color: red;font-size: 18px" class="mdi mdi-file-pdf"></i></a>';
                    } 
                    else if (data == "Line Manager Approval Pending") {
                        if (row.Role_Id == "10") {
                            html += '<a onclick="Edit(' + row.Audit_Sp_Sch_Id + ',' + "'" + 'Edit' + "'" + ',' + "'" + 'Access' + "'" + ')" href="javascript:void(0);" title="Edit" class="text-success" data-bs-original-title="Edit" aria-label="View" style="cursor: pointer;"> <i class="mdi mdi-pencil font-size-18"></i></a>';
                        } else {
                            html += '<a onclick="Edit(' + row.Audit_Sp_Sch_Id + ',' + "'" + 'View' + "'" + ',' + "'" + 'NoAccess' + "'" + ')" href="javascript:void(0);" title="View" class="text-success" data-bs-original-title="View" aria-label="View" style="cursor: pointer;"> <i class="mdi mdi-eye font-size-18"></i></a>';
                        }
                        html += '<a onclick="Fn_Audit_Sp_Report(' + row.Audit_Sp_Sch_Id + ',' + "'" + row.Unique_Id + "'" + ')" style="cursor: pointer;"><i style="color: red;font-size: 18px" class="mdi mdi-file-pdf"></i></a>';
                    }
...
function Fn_Audit_Sp_Report(id, Unique_Id) {
    let NotiID = parseInt(id);
    $(".preloader").show();
    $("#List_View_Div").hide();
    $.post("/AuditReport/Audit_Service_Provider_Report", { Audit_Sp_Sch_Id: NotiID, Unique_Id: Unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
