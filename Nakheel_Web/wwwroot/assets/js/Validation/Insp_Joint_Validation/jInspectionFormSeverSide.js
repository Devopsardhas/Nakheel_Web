function ServerSideTable_Request() {
    let table = $(UI_Fields.MainTable).DataTable({
        ...
        ajax: {
            url: '/Inspection/Joint_Insp_Request_GetAll',
            type: 'POST',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (data == "Inspection Finding Approval Pending") {
                        ...
                        html += '<a onclick="Fn_Joint_Pre_Insp_Report(' + row.Insp_Request_Id + ',' + "'" + row.Unique_Id + "'" + ')" style="cursor: pointer;"><i style="color: red;font-size: 18px" class="mdi mdi-file-pdf"></i></a>';
                    }
                    ...
                    if (data == "Completed") {
                        ...
                        html += '<a onclick="Fn_Joint_Insp_Report(' + row.Insp_Request_Id + ',' + "'" + row.Unique_Id + "'" + ')" style="cursor: pointer;"><i style="color: red;font-size: 18px" class="mdi mdi-file-pdf"></i></a>';
                    }
                    return html;
                }
            },
...
function Fn_Joint_Insp_Report(id, Unique_Id) {
    let NotiID = parseInt(id);
    $(".preloader").show();
    $("#List_View_Div").hide();
    $.post("/InspReport/Joint_Insp_Report", { Insp_Request_Id: NotiID, Unique_Id: Unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
...
function Fn_Joint_Pre_Insp_Report(id, Unique_Id) {
    let NotiID = parseInt(id);
    $(".preloader").show();
    $("#List_View_Div").hide();
    $.post("/InspReport/Joint_Pre_Insp_Report", { Insp_Request_Id: NotiID, Unique_Id: Unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
