function Serversidetable_Electrical(Id) {
    let table = $(UI_Fields.MAINTABLE).DataTable({
        ...
        paging: true,
        ajax: {
            url: '/ControlOfWork/Get_All_Electrical_Work',
            type: 'POST',
        ...
        columns: [
            ...
            { data: 'CreatedDate' },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return '<a href="#" class="' + badge + '" onclick="event.preventDefault();ViewDetails(' + "'" + row.EWP_Id + "'," + "'" + Status + "'" + ')">' + st + '</a>';
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (data == "Zone Supervisor Approval Pending") {
                        html += '<a onclick="Fn_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-warning" data-bs-original-title="View" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                        html += '<a onclick="Fn_EWP_Report_Req(' + row.EWP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Electrical Work Permit Report" class="text-info" data-bs-original-title="Electrical Work Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';

                    }
                    if (data == "HSE Staff/HSE Team Approval Pending") {
                        html += '<a onclick="Fn_View_Electrical_Hse(' + row.EWP_Id + ')"href="javascript:void(0);" title="Review" class="text-primary" data-bs-original-title="View" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';

                    }
                    if (data == "Zone Supervisor Rejected") {
                        html += '<a onclick="Fn_Edit_Electrical(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-danger" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    if (data == "HSE Staff/HSE Team Approved") {
                        html += '<a onclick="Fn_View_Electrical_Hse(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-success" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                        html += '<a onclick="Fn_EWP_Report_Req(' + row.EWP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Electrical Work Permit Report" class="text-info" data-bs-original-title="Confined Space Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    }
                    ...
                    if (data == "Sp uploaded,Waiting for Zone Supervisor Approval") {
                        html += '<a onclick="Fn_Evid_ELEC_Zone_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-primary" data-bs-original-title="Edit" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    if (data == "Renewal Approval Pending") {
                        html += '<a onclick="Fn_Evid_ELEC_Zone_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-warning" data-bs-original-title="Edit" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    if (data == "Zone Supervisor Verified,Renewal Approved") {
                        html += '<a onclick="Fn_Evid_ELEC_Zone_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-success" data-bs-original-title="Edit" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }

                    if (data == "Zone Supervisor Rejected the Renewal") {
                        html += '<a onclick="Fn_Evid_ELEC_Zone_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-danger" data-bs-original-title="Edit" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }

                    if (data == "Zone Supervisor Rejected the Evidence") {
                        html += '<a onclick="Fn_Edit_Evidence_edit(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-danger" data-bs-original-title="Edit" aria-label="Review">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-pencil font-size-18"></i></a>';
                    }
                    ...
                    if (data == "Closed") {
                        html += '<a onclick="Fn_Evid_ELEC_Zone_View(' + row.EWP_Id + ')" href="javascript:void(0);" title="Review" class="text-primary" data-bs-original-title="Edit" aria-label="Edit">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-eye font-size-18"></i></a>';
                        html += '<a onclick="Fn_EWP_Report(' + row.EWP_Id + ',' + "'" + row.Unique_Id + "'" + ')" href="javascript:void(0);" title="Electrical Work Permit Report" class="text-info" data-bs-original-title="Electrical Work Permit Report" aria-label="Final Report">&nbsp;&nbsp;&nbsp;<i class="mdi mdi-file-pdf" style="color:red;font-size:20px;cursor:pointer"></i></a>';
                    }
        ...
        buttons: [
            {
                extend: 'excelHtml5',
                text: 'Excel',
                title: 'Electrical Permit Work List',

            },
            {
                extend: 'pdfHtml5',
                text: 'PDF',
                title: 'Electrical Permit Work'
            },
            {
                extend: 'colvis',
                text: 'Show Columns',
...
function Fn_View(val) {
    ...
    $(UI_Fields.ELECTRICAL_REQUEST_VIEW).show(100);
    Hzurl = '/ControlOfWork/_ViewElectricalWork';
    Hzurl += '/?EWP_Id=' + val;
    $(UI_Fields.VIEW_ELECTRICAL_LIST).load(Hzurl);
}
...
function Fn_View_Electrical_Hse(val) {
    $(UI_Fields.LIST_VIEW).hide(100);
    $(UI_Fields.ELECTRICAL_REQUEST_VIEW).show(100);
    Hzurl = '/ControlOfWork/_ViewElectricalWork';
    Hzurl += '/?EWP_Id=' + val;
...
function Fn_Evid_ELEC_Zone_View(val) {
    $(UI_Fields.LIST_VIEW).hide(100);
    $(UI_Fields.ELECTRICAL_REQUEST_VIEW).show(100);
    Hzurl = '/ControlOfWork/_ViewElectricalWork';
    Hzurl += '/?EWP_Id=' + val;
...
function Fn_Edit_Electrical(val) {
    $(UI_Fields.LIST_VIEW).hide(100);
    $(UI_Fields.ADD_ELECTRICAL_WORK_PERMIT).show(100);
    $(UI_Fields.GOOGLE_MAP_API).show(100);
    Load_GoogleMaps();
    $.post("/ControlOfWork/_EditElectricalWork", { ID: val }, function (data) {
        if (data != null) {
            $("#Electrical_ID").val(data.EWP_Id);
            
            Elec_Get_Edit_Business_Unit("#Ewp_Business_Unit_Id", data.Business_Unit_Id);
            Elec_Get_Edit_Zone_Unit("#Zone_Id", data.Zone_Id, data.Community_Id, data.Building_Id);
            Elec_Get_Edit_DMS_No_Edit("#Contractor_Id", data.DMS_No_Id);
            Elec_Get_Comp_Person_Edit("#C_Competent", data.Competent_Person, data.DMS_No_Id, "#Ewp_Building_Id", data.Building_Id);        
            $("#C_Company_Name").val(data.Company_Name);
            $("#C_Tittle_No").val(data.Contrator_Title_No);
            //Edit_Load_Google_Map(data.Latitude, data.Longitude, data.Location_Address, data.Exact_Loc_Address)
            //$("#C_Competent").val(data.Competent_Person);
            ...
            //$("input[name=C_Approve][value=" + data.Approved_Method + "]").attr('checked', 'checked');
            ...
            $.each(data._Add_CSP_Ques, function (key, value) {
                ...
                $("input[name=" + varYes + "][value=" + value.EWP_Ques_Radio_Btn + "]").prop('checked', true);
                $("#" + varRemark).val(value.EWP_Remarks);
                if (value.EWP_Ques_Radio_Btn == "No" || value.EWP_Ques_Radio_Btn == "NA") {
                    $("#Remarks" + count).show();
                }
                else {
                    $("#Remarks" + count).hide();
...
function Fn_EWP_Report_Req(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Ewp_ID = parseInt(id);
    $.post("/ControlofWorkReport/EWP_Request_Report", { EWP_Id: Ewp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
...
function Fn_EWP_Report(id, unique_Id) {
    $(".preloader").show();
    $(UI_Fields.LIST_VIEW).hide(100);
    let Ewp_ID = parseInt(id);
    $.post("/ControlofWorkReport/EWP_Work_Permit_Report", { EWP_Id: Ewp_ID, Unique_Id: unique_Id }, function (data) {
        if (data) {
            window.open(data);
        }
        else {
